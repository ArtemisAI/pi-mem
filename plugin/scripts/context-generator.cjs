...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let p="";if(n){let{sessionId:T,memorySessionId:S,correlationId:b,...l}=n;Object.keys(l).length>0&&(p=` {${Object.entries(l).map(([E,h])=>`${E}=${h}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${u}${s}${p}${m}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=u?`${u[1].split("/").pop()}:${u[2]}`:"unknown",p={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,p,n),o}},c=new J;var Ut={};function Dt(){return typeof __dirname<"u"?__dirname:(0,f.dirname)((0,ge.fileURLToPath)(Ut.url))}var yt=Dt();function vt(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,f.join)((0,z.homedir)(),".claude-mem"),e=(0,f.join)(r,"settings.json");try{if((0,w.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var N=vt(),L=process.env.CLAUDE_CONFIG_DIR||(0,f.join)((0,z.homedir)(),".claude"),es=(0,f.join)(L,"plugins","marketplaces","thedotmack"),ts=(0,f.join)(N,"archives"),ss=(0,f.join)(N,"logs"),rs=(0,f.join)(N,"trash"),ns=(0,f.join)(N,"backups"),os=(0,f.join)(N,"modes"),is=(0,f.join)(N,"settings.json"),Te=(0,f.join)(N,"claude-mem.db"),as=(0,f.join)(N,"vector-db"),ds=(0,f.join)(N,"observer-sessions"),_s=(0,f.join)(L,"settings.json"),us=(0,f.join)(L,"commands"),cs=(0,f.join)(L,"CLAUDE.md");function fe(r){(0,w.mkdirSync)(r,{recursive:!0})}function Se(){return(0,f.join)(yt,"..")}var be=require("crypto");var xt=3e4;function P(r,e,t){return(0,be.createHash)("sha256").update([r||"",e||"",t||""].join("\0")).digest("hex").slice(0,16)}function H(r,e,t){let s=t-xt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}function Q(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[String(e)]}catch{return[r]}}var O="claude";function kt(r){return r.trim().toLowerCase().replace(/\s+/g,"-")}function M(r){if(!r)return O;let e=kt(r);return e?e==="transcript"||e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("claude")?"claude":new Set(["pi","pi-agent","pi_agent","piagent","pi-coding-agent"]).has(e)?"pi-agent":e:O}function he(r){let e=["claude","codex","cursor","pi-agent"];return[...r].sort((t,s)=>{let n=e.indexOf(t),o=e.indexOf(s);return n!==-1||o!==-1?n===-1?1:o===-1?-1:n-o:t.localeCompare(s)})}function $t(r,e){return{customTitle:r,platformSource:e?M(e):void 0}}var X=class{db;constructor(e=Te){e!==":memory:"&&fe(N),this.db=new Oe.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.addSessionNodeSourceColumn()}initializeSchema(){this.db.run(`
    ...
    `),n||this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_platform_source ON sdk_sessions(platform_source)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(24,new Date().toISOString()))}addObservationModelColumns(){let e=this.db.query("PRAGMA table_info(observations)").all(),t=e.some(n=>n.name==="generated_by_model"),s=e.some(n=>n.name==="relevance_count");t&&s||(t||this.db.run("ALTER TABLE observations ADD COLUMN generated_by_model TEXT"),s||this.db.run("ALTER TABLE observations ADD COLUMN relevance_count INTEGER DEFAULT 0"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(26,new Date().toISOString()))}addSessionNodeSourceColumn(){let t=this.db.query("PRAGMA table_info(sdk_sessions)").all().some(i=>i.name==="node_source"),n=this.db.query("PRAGMA index_list(sdk_sessions)").all().some(i=>i.name==="idx_sdk_sessions_node_source");this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(27)&&t&&n||(t||(this.db.run("ALTER TABLE sdk_sessions ADD COLUMN node_source TEXT NOT NULL DEFAULT 'unknown'"),c.debug("DB","Added node_source column to sdk_sessions table")),this.db.run(`
      UPDATE sdk_sessions
      SET node_source = 'unknown'
      WHERE node_source IS NULL OR node_source = ''
    `),n||this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_node_source ON sdk_sessions(node_source)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(27,new Date().toISOString()))}updateMemorySessionId(e,t){this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,u=s==="date_asc"?"ASC":"DESC",m=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),g=[...e],T=[];if(o&&(T.push("project = ?"),g.push(o)),i)if(Array.isArray(i)){let l=i.map(()=>"?").join(",");T.push(`type IN (${l})`),g.push(...i)}else T.push("type = ?"),g.push(i);if(a){let l=Array.isArray(a)?a:[a],R=l.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");g.push(...l),T.push(`(${R.join(" OR ")})`)}if(d){let l=Array.isArray(d)?d:[d],R=l.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");l.forEach(E=>{g.push(`%${E}%`,`%${E}%`)}),T.push(`(${R.join(" OR ")})`)}let S=T.length>0?`WHERE id IN (${p}) AND ${T.join(" AND ")}`:`WHERE id IN (${p})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${S}
    ...
    `).get(e).count}createSDKSession(e,t,s,n,o,i){let a=new Date,d=a.getTime(),u=$t(n,o),m=u.platformSource??O,p=this.db.prepare(`
    ...
    `).get(e);if(p){if(t&&this.db.prepare(`
        ...
        `).run(u.customTitle,e),i&&this.db.prepare(`
        ...
        `).run(i,e),u.platformSource){let S=p.platform_source?.trim()?M(p.platform_source):void 0;if(!S)this.db.prepare(`
            UPDATE sdk_sessions SET platform_source = ?
            WHERE content_session_id = ?
              AND COALESCE(platform_source, '') = ''
          `).run(u.platformSource,e);else if(S!==u.platformSource)throw new Error(`Platform source conflict for session ${e}: existing=${S}, received=${u.platformSource}`)}return p.id}let g=i||"unknown";return this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, ?, ?, 'active')
    `).run(e,t,m,g,s,u.customTitle||null,a.toISOString(),d),this.db.prepare("SELECT id FROM sdk_sessions WHERE content_session_id = ?").get(e).id}saveUserPrompt(e,t,s){let n=new Date,o=n.getTime();return this.db.prepare(`
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i,a){let d=i??Date.now(),u=new Date(d).toISOString(),m=P(e,s.title,s.narrative),p=H(this.db,m,d);if(p)return{id:p.id,createdAtEpoch:p.created_at_epoch};let T=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d){let u=a??Date.now(),m=new Date(u).toISOString();return this.db.transaction(()=>{let g=[],T=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         ...
         generated_by_model)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let b of s){let l=P(e,b.title,b.narrative),R=H(this.db,l,u);if(R){g.push(R.id);continue}let E=T.run(e,t,b.type,b.title,b.subtitle,JSON.stringify(b.facts),b.narrative,JSON.stringify(b.concepts),JSON.stringify(b.files_read),JSON.stringify(b.files_modified),o||null,i,l,m,u,d||null);g.push(Number(E.lastInsertRowid))}let S=null;if(n){let l=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,m,u);S=Number(l.lastInsertRowid)}return{observationIds:g,summaryId:S,createdAtEpoch:u}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,u,m){let p=u??Date.now(),g=new Date(p).toISOString();return this.db.transaction(()=>{let S=[],b=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
         generated_by_model)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let E of s){let h=P(e,E.title,E.narrative),me=H(this.db,h,p);if(me){S.push(me.id);continue}let Ot=b.run(e,t,E.type,E.title,E.subtitle,JSON.stringify(E.facts),E.narrative,JSON.stringify(E.concepts),JSON.stringify(E.files_read),JSON.stringify(E.files_modified),a||null,d,h,g,p,m||null);S.push(Number(Ot.lastInsertRowid))}let l;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,g,p);l=Number(h.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(p,o),{observationIds:S,summaryId:l,createdAtEpoch:p}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),u=[...e],m=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&u.push(o),this.db.prepare(`
    ...
    `).all(...u)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,u;if(e!==null){let l=`
      ...
      `,R=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let E=this.db.prepare(l).all(e,...a,s+1),h=this.db.prepare(R).all(e,...a,n+1);if(E.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=E.length>0?E[E.length-1].created_at_epoch:t,u=h.length>0?h[h.length-1].created_at_epoch:t}catch(E){return c.error("DB","Error getting boundary observations",void 0,{error:E,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let l=`
      ...
      `,R=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let E=this.db.prepare(l).all(t,...a,s),h=this.db.prepare(R).all(t,...a,n+1);if(E.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=E.length>0?E[E.length-1].created_at_epoch:t,u=h.length>0?h[h.length-1].created_at_epoch:t}catch(E){return c.error("DB","Error getting boundary timestamps",void 0,{error:E,project:o}),{observations:[],sessions:[],prompts:[]}}}let m=`
      SELECT *
      FROM observations
...
`}function _t(r,e,t,s){let n=[];return s?n.push(...Je(r)):n.push(...$e(r)),s?n.push(...ze()):n.push(...Fe()),s?n.push(...Qe()):n.push(...we()),s?n.push(...Ze()):n.push(...Pe()),B(t)&&(s?n.push(...et(e,t)):n.push(...He(e,t))),n}var de=D(require("path"),1);function V(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return c.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function _e(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ue(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function ct(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function ut(r,e){return de.default.isAbsolute(r)?de.default.relative(e,r):r}function mt(r,e,t){let s=V(r);if(s.length>0)return ut(s[0],e);if(t){let n=V(t);if(n.length>0)return ut(n[0],e)}return"General"}function Xt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=ct(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function pt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?V(r.facts).join(`
`):null}function jt(r,e,t,s){let n=[];n.push(...Xe(r));let o="";for(let i of e)if(i.type==="summary"){let a=i.data,d=_e(a.displayTime);n.push(...We(a,d))}else{let a=i.data,d=ue(a.created_at),m=d!==o?d:"";if(o=d,t.has(a.id)){let g=pt(a,s);n.push(...Be(a,m,g,s))}else n.push(Ge(a,m,s))}return n}function Gt(r,e,t,s,n){let o=[];o.push(...tt(r));let i=null,a="";for(let d of e)if(d.type==="summary"){i=null,a="";let u=d.data,m=_e(u.displayTime);o.push(...ot(u,m))}else{let u=d.data,m=mt(u.files_modified,n,u.files_read),p=ue(u.created_at),g=p!==a;a=p;let T=t.has(u.id);if(m!==i&&(o.push(...st(m)),i=m),T){let S=pt(u,s);o.push(...nt(u,p,g,S,s))}else o.push(rt(u,p,g,s))}return o.push(""),o}function Bt(r,e,t,s,n,o){return o?Gt(r,e,t,s,n):jt(r,e,t,s)}function lt(r,e,t,s,n){let o=[],i=Xt(r);for(let[a,d]of i)o.push(...Bt(a,d,e,t,s,n));return o}function Et(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function gt(r,e){let t=[];return e?(t.push(...$("Investigated",r.investigated,_.blue)),t.push(...$("Learned",r.learned,_.yellow)),t.push(...$("Completed",r.completed,_.green)),t.push(...$("Next Steps",r.next_steps,_.magenta))):(t.push(...k("Investigated",r.investigated)),t.push(...k("Learned",r.learned)),t.push(...k("Completed",r.completed)),t.push(...k("Next Steps",r.next_steps))),t}function Tt(r,e){return e?it(r):Ye(r)}function ft(r,e,t){return!B(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?at(r.totalDiscoveryTokens,r.totalReadTokens):qe(r.totalDiscoveryTokens,r.totalReadTokens)}var Wt=St.default.join((0,bt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Yt(){try{return new X}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,ht.unlinkSync)(Wt)}catch(e){c.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return c.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function qt(r,e){return e?dt(r):Ve(r)}function Vt(r,e,t,s,n,o,i){let a=[],d=re(e);a.push(..._t(r,d,s,i));let u=t.slice(0,s.sessionCount),m=Ue(u,t),p=ae(e,m),g=xe(e,s.fullObservationCount);a.push(...lt(p,g,s,n,i));let T=t[0],S=e[0];Et(s,T,S)&&a.push(...gt(T,i));let b=ie(e,s,o,n);return a.push(...Tt(b,i)),a.push(...ft(d,s,i)),a.join(`
