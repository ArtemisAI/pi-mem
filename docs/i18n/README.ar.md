...
قم بتثبيت claude-mem كامتداد للذاكرة المستديمة لـ [pi-agents](https://github.com/badlogic/pi-mono):

```bash
pi install npm:pi-agent-memory
```

يمنح أي وكيل مبني على pi-mono (pi-coding-agent أو OpenClaw أو بيئات التشغيل المخصصة) ذاكرة عبر الجلسات. تُشارك الملاحظات بين جميع المحركات في نفس قاعدة البيانات. راجع [دليل pi-agent](../pi-agent/README.md) للتفاصيل الكاملة.
