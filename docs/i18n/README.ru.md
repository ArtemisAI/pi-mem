...
Установите claude-mem как расширение постоянной памяти для [pi-агентов](https://github.com/badlogic/pi-mono):

```bash
pi install npm:pi-agent-memory
```

Даёт любому агенту на основе pi-mono (pi-coding-agent, OpenClaw, пользовательские среды выполнения) память, сохраняющуюся между сеансами. Наблюдения используются совместно всеми движками в одной базе данных. Подробности см. в [README pi-agent](../pi-agent/README.md).
