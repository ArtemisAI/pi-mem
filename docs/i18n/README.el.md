...
Εγκαταστήστε το claude-mem ως επέκταση μόνιμης μνήμης για [pi-agents](https://github.com/badlogic/pi-mono):

```bash
pi install npm:pi-agent-memory
```

Παρέχει μνήμη μεταξύ συνεδριών σε οποιονδήποτε πράκτορα βασισμένο σε pi-mono (pi-coding-agent, OpenClaw, προσαρμοσμένα runtimes). Οι παρατηρήσεις κοινοποιούνται σε όλους τους κινητήρες στην ίδια βάση δεδομένων. Δείτε το [README του pi-agent](../pi-agent/README.md) για πλήρεις λεπτομέρειες.
