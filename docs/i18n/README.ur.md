...
claude-mem کو [pi-agents](https://github.com/badlogic/pi-mono) کے لیے مستقل میموری توسیع کے طور پر انسٹال کریں:

```bash
pi install npm:pi-agent-memory
```

کسی بھی pi-mono پر مبنی ایجنٹ (pi-coding-agent، OpenClaw، کسٹم رن ٹائمز) کو سیشن کے پار میموری دیتا ہے۔ مشاہدات اسی ڈیٹا بیس میں تمام انجنوں کے درمیان مشترک ہیں۔ مکمل تفصیلات کے لیے [pi-agent README](../pi-agent/README.md) دیکھیں۔
