...
Встановіть claude-mem як розширення постійної пам'яті для [pi-agents](https://github.com/badlogic/pi-mono):

```bash
pi install npm:pi-agent-memory
```

Надає міжсесійну пам'ять будь-якому агенту на основі pi-mono (pi-coding-agent, OpenClaw, власні середовища виконання). Спостереження спільно використовуються всіма рушіями в одній базі даних. Дивіться [README pi-agent](../pi-agent/README.md) для повних деталей.
